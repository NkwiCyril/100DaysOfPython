import random

MAX_LINES = 3
MAX_BET = 50000
MIN_BET = 1

rows = 3
cols = 3
symbols = {
    "A": 2,
    "B": 4,
    "C": 6,
    "D": 8
}

def get_slot_machine_spin(rows, cols, symbols):
    all_symbols = []
    for symbol, symbol_count in symbols.items():
        for _ in range(symbol_count):
            all_symbols.append(symbol)
    
    columns = []
    for _ in range(cols):
        column = []
        current_symbols = all_symbols[:]
        for _ in range(rows):
            value = random.choice(current_symbols)
            current_symbols.remove(value)
            column.append(value)
        columns.append(column)
    return columns  


# Deposit amount into the slot machine
def deposit():
    while True:
        amount = input("What would you like to deposit? XAF")
        if amount.isdigit():
            amount = int(amount)
            if amount > 0:
                break
            else:
                print("Amount must be greater than 0")
        else:
            print("Please enter a valid amount")
    
    return amount

def get_number_of_lines():
    while True:
        lines = input("Enter the number of lines to bet on (1-" + str(MAX_LINES) + ")? ")
        if lines.isdigit():
            lines = int(lines)
            if 1 <= lines <= MAX_LINES:
                break
            else:
                print("Enter a valid number of lines")
        else:
            print("Please enter a valid number")
    
    return lines

def get_bet():
    while True:
        bet = input("What would you like to bet on each line? XAF")
        if bet.isdigit():
            bet = int(bet)
            if bet >= MIN_BET and bet <= MAX_BET:
                break
            else:
                print(f"Amount must be between XAF{MIN_BET} - XAF{MAX_BET}")
        else:
            print("Please enter a valid amount")
    
    return bet

def main():
    balance = deposit()
    lines = get_number_of_lines()

    while True:
        bet = get_bet()
        if balance >= bet * lines:
            break
        else:
            print(f"You do not have enough balance to bet. Your balance is {balance}")

    print(f"You are betting {bet} on {lines} lines. Total bet is equal to {bet * lines}")

    print(balance, lines)


main()